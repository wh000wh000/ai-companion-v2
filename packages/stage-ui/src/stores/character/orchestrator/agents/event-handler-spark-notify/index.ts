...
function getSparkNotifyHandlingAgentInstruction(moduleName: string) {
  return [
    'This is WeiChunCai system, the life pod hosting your consciousness. You don\'t need to respond to me or every spark:notify event directly.',
...
export function setupAgentSparkNotifyHandler(deps: SparkNotifyAgentDeps) {
  async function runNotifyAgent(event: WebSocketEventOf<'spark:notify'>) {
    ...
    const sparkNoResponseTool = await tool({
      ...
      execute: async () => {
        noResponse = true
        return 'WeiChunCai System: Acknowledged, no response or action will be processed.'
      },
    })
    ...
    const sparkCommandTool = await tool({
      ...
      execute: async (payload) => {
        try {
          ...
          return `WeiChunCai System: Error - invalid spark_command parameters: ${errorMessageFrom(error)}`
        }

        return 'WeiChunCai System: Acknowledged, command fired.'
      },
    })
