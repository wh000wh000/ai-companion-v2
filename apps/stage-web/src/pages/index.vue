...
const characterId = ref(localStorage.getItem(CHAR_STORAGE_KEY) || 'default')

// 当 characterId 变更时同步到 localStorage
watch(characterId, (val) => {
  localStorage.setItem(CHAR_STORAGE_KEY, val)
})

// ─── Demo 引导浮层 ───────────────────────────────
const demoStore = useDemoStore()
const GUIDE_DISMISSED_KEY = 'demo_guide_dismissed_day'
...
const showDemoGuide = computed(() => {
  if (!demoStore.isDemo || demoStore.currentDay <= 0)
    return false
  const dismissed = localStorage.getItem(GUIDE_DISMISSED_KEY)
  return dismissed !== String(demoStore.currentDay)
})

const demoDayConfig = computed(() => demoStore.getDayConfig())

function handleGuideDismiss() {
  localStorage.setItem(GUIDE_DISMISSED_KEY, String(demoStore.currentDay))
}

function handleGuideClaimCoins() {
  if (demoStore.checkGiftCoins()) {
    demoStore.markGiftCoinsReceived()
  }
}
...
<template>
  ...
  >
    <div relative flex="~ col" z-2 h-100dvh w-100vw of-hidden>
      ...
      <!-- Gift system floating button + trust display -->
      <GiftFloatingButton :character-id="characterId" />
    </div>

    <!-- Demo 引导浮层（首次访问 / 每日引导） -->
    <DemoGuide
      v-if="showDemoGuide && demoDayConfig"
      :day="demoStore.currentDay"
      :config="demoDayConfig"
      @dismiss="handleGuideDismiss"
      @claim-coins="handleGuideClaimCoins"
      @view-surprise="handleGuideViewSurprise"
      @convert="handleGuideConvert"
    />
  </BackgroundProvider>
</template>
